...
void newname(const char *name)
{
    ...
    //alias("curname", player1->name); // WTF? stef went crazy - this isn't something to translate either.
...
void teaminfo(const char *team, const char *attr)
{
    ...
    loopv(players) if(players[i] && players[i]->team == t)
    {
        ...
        concatformatstring(teammembers, "%s%d ", teammembers, players[i]->clientnum);
    }
    ...
    if(player1->team == t)
    {
        ...
        concatformatstring(teammembers, "%s%d ", teammembers, player1->clientnum);
    }
...
bool showhudtimer(int maxmillis, int startmillis, const char *msg, bool flash)
{
    ...
    defformatstring(str)("\f%s %.1fs", wave ? "1Next wave in" : queued ? "2Queued for spawn:" : "3Waiting for respawn:", (startmillis + maxmillis - lastmillis) / 1000.f);
...
void preparectf(bool cleanonly=false)
{
    ...
    if(!cleanonly)
    {
        loopv(ents)
        {
            ...
            {
                e.spawned = true;
                if(e.attr2>=2) { conoutf("\f3invalid ctf-flag entity (%i as %d)", i, e.attr2); e.attr2 = 0; }
