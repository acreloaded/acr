	...
	while(p.remaining())
	{
		...
		switch(type)
		{
			...
			case N_TEXT:
			{
				...
				else if(cn == -1) chatoutf("\f5[\f1CONSOLE\f5] \f2%s", text);
				break;
			}
			...
			case N_PINGPONG:
			{
				...
				if(multiplayer(false)) addmsg(N_PINGTIME, "i", totalmillis - sentmillis);
			...
			case N_FLAGSECURE:
			{
				...
				ents[ent].attr4 = overthrown; // 1:1 client:server now
				break;
			}
