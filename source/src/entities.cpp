	...
	loopv(ents)
	{
		...
		else if(editmode)
		{
			if(e.type==CTF_FLAG)
			{
				...
				rendermodel("playermodels", ANIM_IDLE|ANIM_TRANSLUCENT|ANIM_END|ANIM_DYNALLOC, -(int)textureload(skin)->id, 1.5f, vec(e.x, e.y, (float)S(e.x, e.y)->floor), e.attr1+90, 0/4);
			}
