...
float raycube(const vec &o, const vec &ray, vec &surface)
{
	...
	for(;;)
	{
		...
		if(dz < dx && dz < dy)
		{
			if(s->type!=FHF && s->type!=CHF) surface.z = ray.z>0 && s->ctex!=DEFAULT_SKY ? -1 : 1;
			dist += dz;
			break;
		}
...
void moveplayer(physent *pl, int moveres, bool local, int curtime)
{
	...
	else // fake physics for player ents to create _the_ cube movement (tm)
	{
		...
		if(pl->type == ENT_PLAYER){
			...
			if(p->weaponsel) d.mul(gunspeed(p->weaponsel->type, p->perk == PERK_SPEED));
		}
		...
		else						// apply velocity with collisions
		{
			if(pl->type!=ENT_CAMERA)
			{
				...
				if(pl->onladder)
				{
					...
					{
						if(p->perk == PERK_CLIMB) climbspeed *= 2;
				...
				else
				{
					if(pl->onfloor || water)
					{
						if(pl->jumpnext)
						{
							...
							else if(p && (p == player1 || p->ownernum == getclientnum())){
