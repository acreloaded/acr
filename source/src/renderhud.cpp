		...
		if(m_secure(gamemode))
		{
			loopv(ents)
			{
				...
				if(e.type == CTF_FLAG && e.attr2 >= 2 && !OUTBORD(ents[i].x, ents[i].y))
				{
					...
					// col: 2: bases, 3: flags
					drawradarent(fixradarpos(pos, centerpos, res), coordtrans, flag_yaw, 2, clamp<int>(ents[i].attr2 - 2, TEAM_RED, TEAM_SPECT), iconsize, 0, owned / 100);
					drawradarent(fixradarpos(pos, centerpos, res), coordtrans, flag_yaw, 2, newteam, iconsize, 0, overthrown / 100);
