...
void drawcrosshair(playerent *p, int n, int teamtype, color *c, float size)
{
	...
	if(n == CROSSHAIR_DEFAULT){
		...
		if(n == CROSSHAIR_SHOTGUN) chsize = SGSPREAD * 100 * (1 - p->ads / 1000.f / SGADSSPREADFACTOR) / dynfov();
...
void gl_drawhud(int w, int h, int curfps, int nquads, int curvert, bool underwater){
	...
	loopv(p->damagestack){
		...
		const float fade = 1 - (lastmillis-pain.millis)/(float)damageindicatorfade, size = damageindicatorsize, dirangle = dir.x ? atan2f(dir.y, dir.x) / RAD : dir.y < 0 ? 270 : 90;
		glPushMatrix();
