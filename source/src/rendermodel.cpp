...
//	else if(d->state==CS_SPAWNING)					{ anim = ANIM_SALUTE|ANIM_LOOP|ANIM_TRANSLUCENT; }
	...
	if(!stenciling && !reflecting && !refracting)
	{
		if(d->weaponsel==d->lastattackweapon && !d->weaponsel->reloading && lastmillis-d->lastaction < d->weaponsel->flashtime())
			anim |= ANIM_PARTICLE;
...
void renderclient(playerent *d)
{
	...
	{
		if(!m_team(gamemode, mutators) || !teamdisplaymode) formatstring(skin)("%s/%s/%02i.jpg", SKINBASE, team_string_old(team), skinid);
		else switch(teamdisplaymode)
		{
			case 1: formatstring(skin)("%s/%s/%02i_%svest.jpg", SKINBASE, team_string_old(team), skinid, team ? "blue" : "red"); break;
			case 2: default: formatstring(skin)("%s/%s/%s.jpg", SKINBASE, team_string_old(team), team ? "blue" : "red"); break;
		}
	}
	string vwep;
