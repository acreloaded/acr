...
void shotevent::process(client *ci)
{
	...
	switch(weap){
		...
		case WEAP_RPG: // explosive tip is stuck to a player
		{
			...
			// instant explosion
			int rpgexplodedmgdealt = dist >= 24 ? explosion(*ci, expc, WEAP_RPG, !m_real(gamemode, mutators), false, hit) : 0;
...
void processevents(){
	loopv(clients)
	{
		...
		if(cs.state == CS_ALIVE){ // can't regen or drown or bleed if dead
			...
			else if(cs.drownmillis > 0) cs.drownmillis = -cs.drownmillis; // save partial drowning
			// bleed = no drown
