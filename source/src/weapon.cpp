...
void damageeffect(int damage, const vec &o){
	particle_splash(3, clamp(damage/10/HEALTHSCALE, 1, 100), 1000, o);
}
...
void weapon::renderhudmodel(int lastaction, bool akimboflip){
	...
	if(gamefocus->protect(totalmillis)) wm.anim |= ANIM_TRANSLUCENT;
...
void setscope(bool enable){
	...
	else if(ads_gun(player1->weaponsel->type) && ads_classic_allowed(player1->weaponsel->type)){
		player1->scoping = enable;
