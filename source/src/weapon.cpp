...
void hit(int damage, playerent *d, playerent *at, const vec &vel, int gun, bool gib, int info){
	...
	if(!m_mp(gamemode)){
		if(d != at && isteam(d, at)){
			dodamage(damage * 0.4, at, at, NUMGUNS, FRAG_GIB, true);
			if((damage *= 0.25) >= d->health) damage = d->health - 1;
		}
		dodamage(damage, d, at, gun, gib ? FRAG_GIB : FRAG_NONE, true);
	}
	else
	{
		hitmsg &h = hits.add();
...
void raydamage(vec &from, vec &to, playerent *d){
	...
	else if((o = intersectclosest(from, to, d, hitzone)))
	{
		...
		if(d==player1){
			...
			else if(hitzone == 3) // legs if not using the slug gun or knife
				dam *= 0.67;
			if(hitzone == 2){
