...
#define TRANSFORMOLDENTITIES(headr) \
        ...
        if(headr.version<7  && strncmp(headr.head,"CUBE",4)==0)  /* only render lights, pl starts and map models on old maps // <6 was <MAPVERSION but we're now at #7 (flowtron) */ \
        { \
            switch(e.type) \
            { \
                case 1: /* old light */ \
        ...
        if(headr.version>=7 && headr.version<8) { \
