...
bool load_world(char *mname)		// still supports all map formats that have existed since the earliest cube betas!
{
	...
	if(gzread(f, &tmp, sizeof(header)-sizeof(int)*16)!=sizeof(header)-sizeof(int)*16) { conoutf("\f3while reading map: header malformatted (base size)"); gzclose(f); return false; }
	endianswap(&tmp.version, sizeof(int), 4);
	...
	if((tmp.version==7 || tmp.version==8) && gzseek(f, sizeof(char)*128, SEEK_CUR)<0) { conoutf("\f3while reading map: header malformatted (1.1 extensions)"); gzclose(f); return false; }
	hdr = tmp;
