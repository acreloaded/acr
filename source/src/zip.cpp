...
struct zipstream : stream
{
    ...
    void stopreading()
    {
        if(reading < 0) return;
#ifndef STANDALONE
        if(dbgzip) conoutf(info->compressedsize ? "%s: zfile.total_out %d, info->size %d" : "%s: reading %d, info->size %d", info->name, int(info->compressedsize ? zfile.total_out : reading - info->offset), info->size);
#endif
